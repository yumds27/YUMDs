...
import {
  ...
  handleRecordSession, handleGetSessions, handleGetSessionDetail,
  handleGetFileProgress, handleMarkFileComplete,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if ((m = pathname.match(/^\/api\/progress\/sessions\/(\d+)$/)) && method === "GET") { response = await handleGetSessionDetail(request, env, m[1]);
    } else if (pathname === "/api/progress/file"           && method === "POST") { response = await handleMarkFileComplete(request, env);
    } else if (pathname === "/api/progress/files"          && method === "GET")  { response = await handleGetFileProgress(request, env);
    // ── Past papers: admin ───────────────────────────────────────────────────
